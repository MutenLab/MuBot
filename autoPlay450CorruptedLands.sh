...
wireSequence=(1 2 3)  # Example: cycles through wires 1,2,3
...
while true; do
  ...
  if [ $buyPotsCounter -eq $buyPotsCycleAt ]; then
    ...
    # GO BACK TO MAP after buying potions
    teleportTo $LOC_CORRUPTED_LANDS
  ...
  if ( [ "$devilSquareEnabled" = true ] && isDevilSquareTime ) || [ "$forceDevilSquare" = true ]; then
    ...
    # GO BACK TO MAP after Devil Square
    teleportTo $LOC_CORRUPTED_LANDS
    teleport_exit_code=$?
  ...
  if ( [ "$bloodCastleEnabled" = true ] && isBloodCastleTime ) || [ "$forceBloodCastle" = true ]; then
    ...
    # GO BACK TO MAP after Blood Castle
    teleportTo $LOC_CORRUPTED_LANDS
    teleport_exit_code=$?
  ...
  echo "[$(date '+%H:%M:%S')] Reposition at w$currentWire..."
  /Users/icerrate/AndroidStudioProjects/bot/bash/travel/corruptedLands/to450GoldenFromEntrance.sh "satan" false &
  reposition_pid=$!
  ...
  if [ $travel_exit_code -eq 1 ]; then
    sleep 5
    echo "[$(date '+%H:%M:%S')] Game recovered. Returning to Map..."
    teleportTo $LOC_CORRUPTED_LANDS
    sleep 1
    # Equip angel from shortcut
    tap_equipAngel
    ...
    echo "[$(date '+%H:%M:%S')] Wrong location detected (likely killed). Returning to Map..."
    ((deathCount++))
    forceBuff=true
    continue
  fi

  # RUN AUTO PLAY AT ZONE
  # ===============================================
  echo "[$(date '+%H:%M:%S')] Arrived to 450 Golden spot..."
