#!/bin/bash
# KEEP OFFENSIVE MODE AT Sanctuary 2 490 golden
...
echo "[$(date '+%H:%M:%S')] Starting auto play at Sanctuary 2. Press key to cancel..."
# Constants for configuration
buyPotsCycleAt=60     # Buy potions every 60 cycles
healthPotions=4500    # Health pots to buy
...
while true; do
  ...
  if ( [ "$devilSquareEnabled" = true ] && isDevilSquareTime ) || [ "$forceDevilSquare" = true ]; then
    ...
    if [ "$event_stopped" = false ]; then
      ((devilSquareCount++))
      echo "[$(date '+%H:%M:%S')] Devil Square completed. Going back to Sanctuary 2..."
    else
      echo "[$(date '+%H:%M:%S')] Devil Square interrupted. Going back to Sanctuary 2..."
    fi
  ...
  if ( [ "$bloodCastleEnabled" = true ] && isBloodCastleTime ) || [ "$forceBloodCastle" = true ]; then
    ...
    if [ "$event_stopped" = false ]; then
      ((bloodCastleCount++))
      echo "[$(date '+%H:%M:%S')] Blood Castle completed. Going back to Sanctuary 2..."
    else
      echo "[$(date '+%H:%M:%S')] Blood Castle interrupted. Going back to Sanctuary 2..."
    fi
  ...
  if [ "$firstReposition" = true ]; then
    /Users/icerrate/AndroidStudioProjects/bot/bash/travel/sanctuary2/to490AGoldenFromEntrance.sh "satan" true &
    reposition_pid=$!
    firstReposition=false
  else
    /Users/icerrate/AndroidStudioProjects/bot/bash/travel/sanctuary2/to490AGoldenFrom490BGolden.sh "satan" true &
    reposition_pid=$!
  fi
  ...
  if [ $travel_exit_code -eq 1 ]; then
    ...
    echo "[$(date '+%H:%M:%S')] Wrong location detected (likely killed). Returning to Sanctuary 2..."
    ((deathCount++))
    forceBuff=true
    firstReposition=true
    needsGoToMap=true
  ...
  # ===============================================
  echo "[$(date '+%H:%M:%S')] Arrived to 490A Golden spot..."
  ...
  # GO TO 490B FROM 490A.
  # ===============================================
  sleep 1
  # Wait for travel to finish, checking for key presses
  # Alternate between recycle+validation and game check
  /Users/icerrate/AndroidStudioProjects/bot/bash/travel/sanctuary2/to490BGoldenFrom490AGolden.sh "satan" true &
  reposition_pid=$!
  while kill -0 $reposition_pid 2>/dev/null; do
    read -t 1 -n 1 key
    if [ $? = 0 ]; then
  ...
  # ===============================================
  echo "[$(date '+%H:%M:%S')] Arrived to 490B Golden spot..."
  /Users/icerrate/AndroidStudioProjects/bot/bash/attack/smartAutoPlay.sh 4 golden &
  cycle_pid=$!                          # Save PID
