#!/bin/bash
# KEEP OFFENSIVE MODE AT Foggy Forest 530A/530B golden
...
while true; do
  ...
  if [ "$buffEnabled" = true ]; then
    ...
    if [ $timeSinceLastBuff -gt 1680 ] || [ "$forceBuff" = true ]; then
      ...
      # GO BACK TO FOGGY FOREST after buffing
      needsGoToMap=false
  ...
  if [ "$FARM_BUY_POTIONS" = true ] && [ $buyPotsCounter -eq $buyPotsCycleAt ]; then
    ...
    # GO BACK TO FOGGY FOREST after buying potions
    needsGoToMap=false
    teleportTo $LOC_FOGGY_FOREST
    teleport_exit_code=$?
  ...
  if ( [ "$devilSquareEnabled" = true ] && isDevilSquareTime ) || [ "$forceDevilSquare" = true ]; then
    ...
    if [ "$event_stopped" = false ]; then
      ((devilSquareCount++))
      echo "[$(date '+%H:%M:%S')] Devil Square completed. Going back to Foggy Forest..."
    else
      echo "[$(date '+%H:%M:%S')] Devil Square interrupted. Going back to Foggy Forest..."
    fi

    # GO BACK TO FOGGY FOREST after Devil Square
    teleportTo $LOC_FOGGY_FOREST
    teleport_exit_code=$?
  ...
  if ( [ "$bloodCastleEnabled" = true ] && isBloodCastleTime ) || [ "$forceBloodCastle" = true ]; then
    ...
    if [ "$event_stopped" = false ]; then
      ((bloodCastleCount++))
      echo "[$(date '+%H:%M:%S')] Blood Castle completed. Going back to Foggy Forest..."
    else
      echo "[$(date '+%H:%M:%S')] Blood Castle interrupted. Going back to Foggy Forest..."
    fi

    # GO BACK TO FOGGY FOREST after Blood Castle
    teleportTo $LOC_FOGGY_FOREST
    teleport_exit_code=$?
  ...
  if [ "$firstReposition" = true ]; then
    $PROJECT_DIR/bash/travel/foggyForest/toGolden530AFromEntrance.sh "satan" &
    reposition_pid=$!
    firstReposition=false
  else
    $PROJECT_DIR/bash/travel/foggyForest/toGolden530AFrom530B.sh &
    reposition_pid=$!
  fi
  ...
  if [ $travel_exit_code -eq 1 ]; then
    ...
    echo "[$(date '+%H:%M:%S')] Wrong location detected (likely killed). Returning to Foggy Forest..."
    ((deathCount++))
    forceBuff=true
    firstReposition=true
    needsGoToMap=true
  ...
  # RUN AUTO PLAY AT 530A GOLDEN
  # ===============================================
  echo "[$(date '+%H:%M:%S')] Arrived to 530A Golden spot..."
  ...
  # GO TO 530B GOLDEN FROM 530A.
  # ===============================================
  sleep 1
  # Wait for travel to finish, checking for key presses
  $PROJECT_DIR/bash/travel/foggyForest/toGolden530BFrom530A.sh &
  reposition_pid=$!
  while kill -0 $reposition_pid 2>/dev/null; do
    read -t 1 -n 1 key
    if [ $? = 0 ]; then
  ...
  # RUN AUTO PLAY AT 530B GOLDEN
  # ===============================================
  echo "[$(date '+%H:%M:%S')] Arrived to 530B Golden spot..."
  $PROJECT_DIR/bash/attack/smartAutoPlay.sh golden &
  cycle_pid=$!                          # Save PID
