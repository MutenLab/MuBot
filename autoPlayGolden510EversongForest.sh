#!/bin/bash
# KEEP OFFENSIVE MODE AT Eversong Forest 510 golden
...
buyPotsCycleAt=100    # Buy potions every 100 cycles
...
wireSequence=(1 2 3)    # Cycles through wires 1,2,3 to kill golden
pauseFlagFile="/tmremp/mubot_paused"
...
while true; do
  ...
  if [ $buyPotsCounter -eq $buyPotsCycleAt ]; then
    ...
    # GO BACK TO EVERSONG FOREST after buying potions
    teleportTo $LOC_EVERSONG_FOREST
  ...
  if [ "$buffEnabled" = true ]; then
    ...
    if [ $timeSinceLastBuff -gt 1680 ] || [ "$forceBuff" = true ]; then
      ...
      # GO BACK TO EVERSONG FOREST after buffing
      teleportTo $LOC_EVERSONG_FOREST
      teleport_exit_code=$?
  ...
  if ( [ "$devilSquareEnabled" = true ] && isDevilSquareTime ) || [ "$forceDevilSquare" = true ]; then
    ...
    if [ "$event_stopped" = false ]; then
      ((devilSquareCount++))
      echo "[$(date '+%H:%M:%S')] Devil Square completed. Going back to Eversong Forest..."
    else
      echo "[$(date '+%H:%M:%S')] Devil Square interrupted. Going back to Eversong Forest..."
    fi

    # GO BACK TO EVERSONG FOREST after Devil Square
    teleportTo $LOC_EVERSONG_FOREST
    teleport_exit_code=$?
  ...
  if ( [ "$bloodCastleEnabled" = true ] && isBloodCastleTime ) || [ "$forceBloodCastle" = true ]; then
    ...
    if [ "$event_stopped" = false ]; then
      ((bloodCastleCount++))
      echo "[$(date '+%H:%M:%S')] Blood Castle completed. Going back to Eversong Forest..."
    else
      echo "[$(date '+%H:%M:%S')] Blood Castle interrupted. Going back to Eversong Forest..."
    fi

    # GO BACK TO EVERSONG FOREST after Blood Castle
    teleportTo $LOC_EVERSONG_FOREST
    teleport_exit_code=$?
  ...
  sleep 5
    echo "[$(date '+%H:%M:%S')] Game recovered. Returning to Eversong Forest..."
    teleportTo $LOC_EVERSONG_FOREST
    sleep 1
    tap_equipSatan
    sleep 1
